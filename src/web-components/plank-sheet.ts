...
/**
 * PlankSheet - Root container that manages sheet state
 *
 ...
 *     <button>Open</button>
 *   </plank-sheet-trigger>
 *   <plank-sheet-content side="right">
 ...
 * ```
 */
@customElement("plank-sheet")
export class PlankSheet extends LitElement {
  @property({ type: Boolean, reflect: true }) open = false

  private _trigger: PlankSheetTrigger | null = null
  private _content: PlankSheetContent | null = null
  private _sheetId = `plank-sheet-${++sheetId}`
  ...
  private _updateChildren() {
    this._trigger = this.querySelector(
      "plank-sheet-trigger"
    ) as PlankSheetTrigger | null
    this._content = this.querySelector(
      "plank-sheet-content"
    ) as PlankSheetContent | null
...
/**
 * PlankSheetTrigger - Element that triggers the sheet on click
 */
@customElement("plank-sheet-trigger")
export class PlankSheetTrigger extends LitElement {
  private _sheet: PlankSheet | null = null
...
/**
 * PlankSheetClose - Element that closes the sheet on click
 */
@customElement("plank-sheet-close")
export class PlankSheetClose extends LitElement {
  private _sheet: PlankSheet | null = null
  ...
  private _handleClick = () => {
    ...
    const sheet = this.closest("plank-sheet") as PlankSheet | null
    sheet?._close()
  }
...
/**
 * PlankSheetContent - The sheet content with overlay
 */
@customElement("plank-sheet-content")
export class PlankSheetContent extends LitElement {
  ...
  private _showContent() {
    ...
    const title = this._contentEl.querySelector("plank-sheet-title")
    if (title) {
      const titleEl = title.querySelector("h2") || title
      titleEl.id = this._titleId
    }

    const description = this._contentEl.querySelector("plank-sheet-description")
    if (description) {
...
/**
 * PlankSheetHeader - Header container for title and description
 */
@customElement("plank-sheet-header")
export class PlankSheetHeader extends LitElement {
  @property({ type: String }) class: string = ""
...
/**
 * PlankSheetFooter - Footer container for actions
 */
@customElement("plank-sheet-footer")
export class PlankSheetFooter extends LitElement {
  @property({ type: String }) class: string = ""
...
/**
 * PlankSheetTitle - Sheet title
 */
@customElement("plank-sheet-title")
export class PlankSheetTitle extends LitElement {
  @property({ type: String }) class: string = ""
...
/**
 * PlankSheetDescription - Sheet description
 */
@customElement("plank-sheet-description")
export class PlankSheetDescription extends LitElement {
  @property({ type: String }) class: string = ""
...
declare global {
  interface HTMLElementTagNameMap {
    "plank-sheet": PlankSheet
    "plank-sheet-trigger": PlankSheetTrigger
    "plank-sheet-close": PlankSheetClose
    "plank-sheet-content": PlankSheetContent
    "plank-sheet-header": PlankSheetHeader
    "plank-sheet-footer": PlankSheetFooter
    "plank-sheet-title": PlankSheetTitle
    "plank-sheet-description": PlankSheetDescription
  }
}
